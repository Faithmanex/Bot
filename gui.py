...
<div id="controls">
  ...
  <button class="stop-btn" onclick="prevTrade()" title="Previous trade" style="padding:4px 9px;font-size:10px;min-width:32px">◀ TRADE</button>
  <button class="sp" onclick="nextTrade()" title="Next trade" style="padding:4px 9px;font-size:10px;min-width:32px">TRADE ▶</button>
...
function nextTrade() {{
  ...
  const sorted = activeTrades
    .filter(t => t.sigIdx !== -1)
    .sort((a, b) => a.sigIdx - b.sigIdx);
  const next = sorted.find(t => t.sigIdx > rIdx);
  if (!next) return;
  const target = next.entryIdx !== -1 ? Math.max(0, next.entryIdx - 10) : Math.max(0, next.sigIdx - 5);
  resetReplay();
  advance(target);
  chart.timeScale().scrollToRealTime();
}}
...
function prevTrade() {{
  ...
  const sorted = activeTrades
    .filter(t => t.sigIdx !== -1)
    .sort((a, b) => a.sigIdx - b.sigIdx);
  const prev = sorted.reverse().find(t => t.sigIdx < rIdx);
  if (!prev) return;
  const target = prev.entryIdx !== -1 ? Math.max(0, prev.entryIdx - 10) : Math.max(0, prev.sigIdx - 5);
  resetReplay();
  advance(target);
  chart.timeScale().scrollToRealTime();
}}
    ...
    def _export_btshare(self):
        ...
        for tf in ["M1","M5","M10","M15","M30","H1","H4","D1"]:
            if fname.endswith(f"_{tf}.html"):
                symbol = fname.replace("live_replay_", "").replace(f"_{tf}.html", "")
                timeframe_name = tf
                break
        ...
        try:
            ...
            with zipfile.ZipFile(share_path, "w", zipfile.ZIP_DEFLATED) as zf:
                zf.writestr("data.json", _json.dumps(payload, ensure_ascii=False))
            ...
            self.log_message(
                ...
                f"       Viewer: https://bot-eight-flax.vercel.app\n",
                "INFO"
            )
        except Exception as exc:
