...
class Strategy:
    ...
    def Noir(self, RR):
        ...
        for x in range(self.new_df.shape[0]):
            if (
                 # Check if SuperTrend is in an uptrend
                pd.notna(self.new_df.iloc[x]["Is_High"])
                and pd.notna(self.new_df.iloc[x - 1]["Is_Low"])
                and pd.notna(self.new_df.iloc[x - 2]["Is_High"])
                and pd.notna(self.new_df.iloc[x - 3]["Is_Low"])
                and pd.notna(self.new_df.iloc[x - 4]["Is_High"])
                and pd.notna(self.new_df.iloc[x - 5]["Is_Low"])
            ):
                current_high = self.new_df.iloc[x]["Is_High"]
                high_2 = self.new_df.iloc[x - 2]["Is_High"]
                high_4 = self.new_df.iloc[x - 4]["Is_High"]
                low_1 = self.new_df.iloc[x - 1]["Is_Low"]
                low_3 = self.new_df.iloc[x - 3]["Is_Low"]
                low_5 = self.new_df.iloc[x - 5]["Is_Low"]
                if (
                    current_high > high_2
                    and high_4 > high_2
                    and low_1 < low_3
                    and high_4 < current_high
                    and (high_2 - current_high) > (110 / 100 * (high_2 - low_1))
                    and (high_2 - current_high) < (300 / 100 * (high_2 - low_1))
                    and (low_3 - high_2) > (120 / 100 * (low_3 - high_4))
                    and (low_3 - high_2) < (300 / 100 * (low_3 - high_4))

                ):
                    self.occurences.append(self.new_df.index[x])
                    block_range = high_4 - low_3
                    entry_price = ((high_2 - low_1)/2) + low_1
                    self.entries.append(entry_price)
